...
function ItemQTYButton({ state, setState, product, forCart }) {
    ...
    return (
        <div className="center h-1/2 w-fit  my-2 bg-gray-50 rounded-full  overflow-hidden relative">
            <h1 className={`${forCart ? 'text-white' : ''} font-light absolute w-full text-center -top-5`}>Quntity</h1>
            <Button onClick={() => { controlQTY('sub') }} className="rounded-none rounded-l min-w-0 h-8 w-10  bg-gray-300 center"><ImMinus /> </Button>
            ...
            <Button onClick={() => { controlQTY() }} className=" rounded-none  min-w-0 h-8 w-10 bg-gray-300 center"><IoPulse /> </Button>
